...
function App() {
  ...
  let [properties, setProperties] = useState(allProperties);

  function searchProperties(e) {
    e.preventDefault();

    const locationQuery = locationRef.current.value.split(", ")[0];
    const guestQuery = guestRef.current.value || 0;
    ...
    <>
      ...
      <main>
        ...
        <section className="all-properties">
          {!properties.length && <p>Sorry, no stays available</p>}
          {properties.map((property, i) => (
            <Property
              key={i}
              imgUrl={property.photo}
